    ...
    for (double yi=4.0; yi<=9.0; yi+=0.1){
        ...
        for (int i=0; i<=90; i++) {
            ...
            for (int j=1; j<=ODE2.kount; j++) {
				if (abs(this->ODE2.get_x(j) - 10.0) < 1e-6) {
					fprintf(this->fp, "%lg %lg %lg %lg\n", this->ODE2.get_x(j), log10(this->ODE2.get_y(1,j)), flux, this->yi);
				}
            }
            for (int j=1; j<=ODE.kount; j++) {
				if (abs(this->ODE.get_x(j) - 10.0) < 1e-6) {
					fprintf(this->fp, "%lg %lg %lg %lg\n", this->ODE.get_x(j), log10(this->ODE.get_y(1,j)), flux, this->yi);
				}
            }
        }
		printf("hi\n");
...
// for the specified flux, integrate inwards to see if we match the base temperature
{
	...
	this->ODE.go_simple(yi,10.0,(int)((10.0-yi)/0.1));
